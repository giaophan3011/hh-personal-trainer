...
import {
  deleteTrainingMiddleware,
  getTrainingsMiddleware,
} from "../redux/middleware/trainingMiddleware";
...
const Row = (props) => {
  ...
  let customerName =
    row.customer == null ? "" : `${row.customer.firstname} ${row.customer.lastname}`;
    ...
    <React.Fragment>
      <TableRow>
        ...
        <TableCell>
          <DeleteForeverIcon
            color="secondary"
            onClick={() => dispatch(displayConfirmDialog("Delete training ", row, deleteTraining))}
          />
        </TableCell>
      </TableRow>
    </React.Fragment>
    ...
    <div>
      <EnhancedTable
        ...
        filterFunction={(row, searchValue) =>
          ...
          row.duration.toString().includes(searchValue) ||
          row.activity.includes(searchValue) ||
          (row.customer !== null &&
            `${row.customer.firstname} ${row.customer.lastname}`.includes(searchValue))
        }
      />
    </div>
  );
