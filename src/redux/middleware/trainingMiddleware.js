...
const {
  ...
  deleteTrainingById,
  addNewTraining,
} = require("../../services/trainingApi");
const {
  getTrainingsAction,
  deleteTrainingAction,
  addTrainingAction,
} = require("../actions/trainingActions");

export function getTrainingsMiddleware() {
  return function (dispatch) {
    return getAllTrainings().then((data) => dispatch(getTrainingsAction(data)));
  };
}

export function deleteTrainingMiddleware(training) {
  return function (dispatch) {
    return deleteTrainingById(training)
      .then((data) => {
